...
class DefaultRelayProbeService implements RelayProbeService {
  ...
  /// A candidate needs TLS when:
  ///   - extras['tls'] == true (explicit flag), or
  ///   - a Reality block is present (reality-opts / reality), or
  ///   - the type is trojan / anytls (TLS-only protocols).
  ///
  /// Naked VLESS (no tls, no reality) does NOT force TLS — some
  /// deployments use VLESS over plain TCP on LAN or via a separate
  /// transport. We probe what the candidate actually declared, not what
  /// we wish it declared.
  ///
  /// Reality detection mirrors [ProtocolRanker._hasReality]. The two
  /// copies must stay in sync; there is a test ensuring they agree on the
  /// canonical shapes.
  static bool _needsTls(RelayCandidate c) {
    if (c.extras['tls'] == true) return true;
    if (_hasReality(c.extras)) return true;
    final t = c.type.toLowerCase();
    if (t == 'trojan' || t == 'anytls') return true;
    return false;
  }
