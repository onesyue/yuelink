...
class _SplitTunnelSectionState extends ConsumerState<SplitTunnelSection> {
  ...
  String _modeTitle(S s, SplitTunnelMode mode) {
    switch (mode) {
      case SplitTunnelMode.whitelist:
        return s.splitTunnelModeWhitelist;
      case SplitTunnelMode.blacklist:
        return s.splitTunnelModeBlacklist;
      case SplitTunnelMode.all:
        return s.splitTunnelModeAll;
    }
  }

  Future<void> _pickMode(BuildContext context, SplitTunnelMode current) async {
    final s = S.of(context);
    final picked = await showYLSettingsOptionPicker<SplitTunnelMode>(
      context: context,
      title: s.splitTunnelMode,
      selectedValue: current,
      options: [
        YLSettingsOption(
          value: SplitTunnelMode.all,
          title: s.splitTunnelModeAll,
        ),
        YLSettingsOption(
          value: SplitTunnelMode.whitelist,
          title: s.splitTunnelModeWhitelist,
        ),
        YLSettingsOption(
          value: SplitTunnelMode.blacklist,
          title: s.splitTunnelModeBlacklist,
        ),
      ],
    );
    if (picked == null || picked == current) return;
    ref.read(splitTunnelModeProvider.notifier).set(picked);
  }
      ...
      child: Column(
        children: [
          ...
          YLInfoRow(
            ...
            trailing: YLSettingsValueButton(label: _modeTitle(s, mode)),
